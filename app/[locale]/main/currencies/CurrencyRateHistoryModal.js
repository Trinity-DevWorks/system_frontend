...
export default function CurrencyRateHistoryModal({ open, currency, onClose }) {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["tenant", "currencies", currencyId, "rate-history", fromStr ?? "", toStr ?? ""],
    queryFn: () =>
  ...
  const pairs = useMemo(() => {
    const raw = data && typeof data === "object" && "pairs" in data ? data.pairs : null;
    return Array.isArray(raw) ? raw : [];
  }, [data]);

  const effectiveSelectedToId =
    selectedToId || (pairs[0]?.to_currency?.id != null ? String(pairs[0].to_currency.id) : "");

  const selectedPair = useMemo(() => {
    if (!effectiveSelectedToId) return pairs[0] ?? null;
    return pairs.find((p) => String(p?.to_currency?.id) === String(effectiveSelectedToId)) ?? null;
  ...
  const formatDate = (iso) => {
    ...
    try {
      ...
      return Number.isFinite(d.getTime()) ? d.toLocaleDateString(locale, { dateStyle: "short" }) : "\u2014";
    } catch {
      return "\u2014";
    }
  };
  ...
  const titleName = typeof currency?.code === "string" ? currency.code : String(currencyId ?? "");
  ...
  return (
    ...
    >
      {isLoading ? (
        ...
        <Typography.Text type="secondary">{t("rateHistoryEmpty")}</Typography.Text>
      ) : (
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <Select
              ...
              options={pairs.map((p) => ({
                ...
                label: p.to_currency?.code ?? p.to_currency?.name ?? String(p.to_currency?.id),
              }))}
            />
          </div>
          {selectedPair ? (
            <Typography.Text type="secondary" className="text-sm">
              1 {currency?.code} = {String(selectedPair.current_rate)} {selectedPair.to_currency?.code}
            </Typography.Text>
          ) : null}
          <Table
            size="small"
            ...
            rowKey="key"
            columns={[
              { title: t("rateHistoryPeriod"), dataIndex: "period", key: "period" },
              { title: t("rateHistoryRate"), dataIndex: "rate", key: "rate", align: "right" },
              { title: t("rateHistoryUpdatedBy"), dataIndex: "updated_by", key: "updated_by" },
            ]}

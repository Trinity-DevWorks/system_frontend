 ...
 * }} args
 */
export function useWarehouseDrawerMutations({ form, message, t, tApiErrors, onClose, onCreated, defaults }) {
  ...
  const createMutation = useMutation({
    ...
    onMutate: async ({ payload }) => {
      ...
      queryClient.setQueryData(listKey, (old) => {
        ...
        return sortWarehousesByName([...base, optimisticRow]);
      });
  ...
  const updateMutation = useMutation({
    ...
    onError: (err, _variables, context) => {
      if (context?.previousList !== undefined) {
        queryClient.setQueryData(["tenant", "warehouses"], context.previousList);
      }
    ...
    onSuccess: (data, { id }) => {
      ...
      queryClient.setQueryData(listKey, (old) => {
        if (!Array.isArray(old)) return old;
        return sortWarehousesByName(old.map((row) => (row.id === id ? data : row)));
      });

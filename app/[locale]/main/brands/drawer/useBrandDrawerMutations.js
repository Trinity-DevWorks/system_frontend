...
import { createBrand, updateBrand } from "@/services/brandsApi";
 ...
 * }} args
 */
export function useBrandDrawerMutations({
  form,
  message,
  t,
  tApiErrors,
  ...
  const createMutation = useMutation({
    ...
    onMutate: async ({ payload }) => {
      ...
      queryClient.setQueryData(listKey, (old) => {
        ...
        return sortBrandsByName([...base, optimisticRow]);
      });
    ...
    onSuccess: (data, variables, context) => {
      ...
      if (id != null) {
        queryClient.setQueryData(["tenant", "brands", id], data);
      }
  ...
  const updateMutation = useMutation({
    ...
    onMutate: async ({ id, values }) => {
      const listKey = ["tenant", "brands"];
      const detailKey = ["tenant", "brands", id];
      await queryClient.cancelQueries({ queryKey: listKey });
    ...
    onError: (err, _variables, context) => {
      if (context?.previousList !== undefined) {
        queryClient.setQueryData(["tenant", "brands"], context.previousList);
      }
      if (context?.previousDetail !== undefined) {
        queryClient.setQueryData(["tenant", "brands", context.id], context.previousDetail);
      }
      if (!applyApiFieldErrors(form, err)) {
        message.error(getLocalizedApiErrorMessage(tApiErrors, err));
      }
    ...
    onSuccess: (data, { id }) => {
      ...
      queryClient.setQueryData(listKey, (old) => {
        if (!Array.isArray(old)) return old;
        return sortBrandsByName(old.map((row) => (row.id === id ? data : row)));
      });
    ...
    onSettled: (_data, _error, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["tenant", "brands"] });
      if (id != null) {
        queryClient.invalidateQueries({ queryKey: ["tenant", "brands", id] });
      }
    },
  });

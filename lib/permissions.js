 ...
 *
 * Permission flags are withheld until after mount so SSR HTML matches the first
 * client render (token + React Query cache are browser-only).
 */
export function usePermissions() {
  ...
  const hasToken = hydrated && Boolean(getSessionToken("tenant"));
  ...
  const matrix = hydrated ? query.data ?? null : null;
  ...
  return {
    ...
    can,
    isLoading:
      !hydrated || (hasToken && query.isPending && query.data == null),
    isError: hydrated && query.isError,
    isReady: hydrated && matrix != null,

 ...
 * Field-level validation messages win over the generic `VALIDATION_ERROR`
 * translation so business-rule failures stay precise in toasts.
 * Other known `ApiErrors.codes.*` keys still win over English API text.
 * @param {(key: string, values?: Record<string, unknown>) => string} tErrors `useTranslations("ApiErrors")`
 * @param {unknown} error
 * @returns {string}
 */
export function getLocalizedApiErrorMessage(tErrors, error) {
  ...
  if (code) {
    ...
    if (typeof tErrors.has === "function" && tErrors.has(key)) {
      return tErrors(key);
    }
    try {
      ...
      // Unknown code key, continue to fallbacks.
    }
  }

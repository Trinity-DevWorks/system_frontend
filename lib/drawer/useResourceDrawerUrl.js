...
export function useResourceDrawerUrl(options = {}) {
  ...
  // Seed is only valid for the record currently in the URL. Closing the drawer
  // or landing on a different id (notification / refresh) just ignores a stale
  // seed instead of clearing it in an effect.
  const resolvedTableSeed = useMemo(() => {
    if (!derived.open || derived.recordId == null || !tableSeed) return null;
    return normalizeEntityId(tableSeed.id) === String(derived.recordId) ? tableSeed : null;
  }, [derived.open, derived.recordId, tableSeed]);

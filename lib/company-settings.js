...
export function companySettingsQueryKey(hostname) {
  return ["tenant", "company-settings", hostname];
}

/** Other tabs listen for this key and refetch company settings. */
export const COMPANY_SETTINGS_SYNC_KEY = "tenant:company-settings:rev";

export function bumpCompanySettingsSync() {
  try {
    localStorage.setItem(COMPANY_SETTINGS_SYNC_KEY, String(Date.now()));
  } catch {
...
export function normalizeCompanySettings(payload) {
  ...
  return {
    ...
    taxPriceMode: isTaxPriceMode(raw.tax_price_mode)
      ? raw.tax_price_mode
      : DEFAULT_COMPANY_SETTINGS.taxPriceMode,
    allowNegativeStock:
    ...
    inventoryCostingMethod: isInventoryCostingMethod(raw.inventory_costing_method)
      ...
      : DEFAULT_COMPANY_SETTINGS.inventoryCostingMethod,
    priceRoundingMode,
    priceDecimalPlaces,
  };
}
...
export function useCompanySettings() {
  ...
  useEffect(() => {
    const onStorage = (event) => {
      if (event.key !== COMPANY_SETTINGS_SYNC_KEY) return;
      queryClient.invalidateQueries({ queryKey: ["tenant", "company-settings"] });
    };
 ...
 *
 * @param {CompanySettings | null | undefined} settings
 */

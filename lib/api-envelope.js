/**
 * Normalizes backend API envelope responses.
 * Used to detect success/error payloads and extract user-safe error details.
 */
/**
...
/**
 * @param {unknown} body
 * @returns {{ message: string, code?: string, errors?: Record<string, string[]> }}
 */
export function getApiErrorDisplay(body) {
  ...
  const code =
    "code" in body && typeof body.code === "string" && body.code.trim()
      ? body.code.trim()
      : undefined;
  const errors =
    "errors" in body &&
    ...
    body.errors !== null
      ...
      : undefined;
  return { message: body.message, code, errors };
}

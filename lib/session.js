/**
 * Auth token cookie utilities.
 * Used to set/get/clear central and tenant session tokens in the browser.
 */
...
/** Must match attributes used in `cookieOptions` or the browser may keep the cookie. */
...
export function clearSessionToken(mode) {
  const key = mode === "central" ? CENTRAL_TOKEN_KEY : TENANT_TOKEN_KEY;
  Cookies.remove(key, cookieRemovalOptions);
}

export function clearAllSessionTokens() {
  Cookies.remove(CENTRAL_TOKEN_KEY, cookieRemovalOptions);
  Cookies.remove(TENANT_TOKEN_KEY, cookieRemovalOptions);
}

...
export async function fetchRecipe(itemId) {
  try {
    return await tenantApiService("GET", `items/${itemId}/recipe`);
  } catch (err) {
...
export function upsertRecipe(itemId, body) {
  return tenantApiService("PUT", `items/${itemId}/recipe`, body);
}
...
export async function fetchRecipeItems(itemId) {
  const data = await tenantApiService("GET", `items/${itemId}/recipe-items`);
...
export function addRecipeItem(itemId, body) {
  return tenantApiService("POST", `items/${itemId}/recipe-items`, body);
}
...
export async function syncRecipeItems(itemId, body) {
  const data = await tenantApiService("PUT", `items/${itemId}/recipe-items/sync`, body);
  return Array.isArray(data) ? data : [];
}
...
export function updateRecipeItem(itemId, recipeItemId, body) {
  return tenantApiService("PUT", `items/${itemId}/recipe-items/${recipeItemId}`, body);
}
...
export function deleteRecipeItem(itemId, recipeItemId) {
  return tenantApiService("DELETE", `items/${itemId}/recipe-items/${recipeItemId}`);
}

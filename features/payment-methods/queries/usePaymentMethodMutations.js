...
import { createPaymentMethod, updatePaymentMethod } from "../api/paymentMethods.api";
import {
...
import { PAYMENT_METHODS_LIST_QUERY_KEY, paymentMethodDetailQueryKey } from "./paymentMethodsQueryKeys";
 ...
 * }} args
 */
export function usePaymentMethodDrawerMutations({
  form,
  message,
  t,
  tApiErrors,
  ...
  const createMutation = useMutation({
    ...
    onMutate: async ({ payload }) => {
      const listKey = PAYMENT_METHODS_LIST_QUERY_KEY;
      ...
      patchTenantListCacheForCreate(queryClient, listKey, (rows) => sortPaymentMethodsByName([...rows, optimisticRow]));
    ...
    onSuccess: (data, variables, context) => {
      ...
      if (id != null) {
        queryClient.setQueryData(paymentMethodDetailQueryKey(id), data);
      }
    ...
    onSettled: () => {
      invalidateTenantListQueries(queryClient, PAYMENT_METHODS_LIST_QUERY_KEY);
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onMutate: async ({ id, values }) => {
      const listKey = PAYMENT_METHODS_LIST_QUERY_KEY;
      const detailKey = paymentMethodDetailQueryKey(id);
      await cancelTenantListQueries(queryClient, listKey);
    ...
    onError: (err, _variables, context) => {
      ...
      if (context?.previousDetail !== undefined) {
        queryClient.setQueryData(paymentMethodDetailQueryKey(context.id), context.previousDetail);
      }
      if (!applyApiFieldErrors(form, err)) {
        message.error(getLocalizedApiErrorMessage(tApiErrors, err));
      }
    ...
    onSuccess: (data, { id }) => {
      const listKey = PAYMENT_METHODS_LIST_QUERY_KEY;
      const detailKey = paymentMethodDetailQueryKey(id);
      patchTenantListCache(queryClient, listKey, (rows) => sortPaymentMethodsByName(rows.map((row) => (row.id === id ? data : row))));
    ...
    onSettled: (_data, _error, variables) => {
      ...
      invalidateTenantListQueries(queryClient, PAYMENT_METHODS_LIST_QUERY_KEY);
      if (id != null) {
        queryClient.invalidateQueries({ queryKey: paymentMethodDetailQueryKey(id) });
      }
    },
  });

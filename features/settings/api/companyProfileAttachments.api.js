...
} from "@/lib/attachments/attachmentBlob";
...
export async function fetchCompanyProfileAttachments() {
  const data = await tenantRequest("GET", "company-profile/attachments");
...
export function uploadCompanyProfileAttachment(file, config = {}) {
  ...
  fd.append("file", file);
  return tenantRequest("POST", "company-profile/attachments", fd, config);
}

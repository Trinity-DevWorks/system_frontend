...
import { BRANDS_LIST_QUERY_KEY } from "../../queries/brandsQueryKeys";
import { useBrandMutations } from "../../queries/useBrandMutations";
import { useBrandNamesQuery } from "../../queries/useBrandNamesQuery";
import {
...
export default function BrandDrawer({
  ...
  const { detailEnabled, tableSeedMatches, fetchRemoteDetail, detailQuery } = useResourceDrawerDetailSync({
    ...
    form,
    defaults,
    queryKeyPrefix: BRANDS_LIST_QUERY_KEY,
    fetchDetail: fetchBrand,
    mapSeedToCacheRow,
    mapRecordToFormValues,
  });

  const brandsQuery = useBrandNamesQuery({ enabled: open });
  ...
  const { createMutation, updateMutation, applyPayload, submitting } = useBrandMutations({
    form,
    message,
    t,
    tApiErrors,

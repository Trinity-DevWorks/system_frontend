 ...
 * }} [actions]
 */
export function getBundleExplosionTableColumns(t, actions = {}) {
  const { onView, onEdit, onDelete } = actions;

  return [
    {
      title: t("colBexNumber"),
      dataIndex: "bex_number",
      key: "bex_number",
      width: 140,
      render: (value) => value || "—",
    },
    ...
    },
    {
      ...
      width: 110,
      render: (value) => (
        <Tag color={value === "posted" ? "green" : "default"}>{getBundleExplosionStatusLabel(t, value)}</Tag>
      ),
    },
    {
      title: t("colExplosionDate"),
      dataIndex: "explosion_date",
      key: "explosion_date",
      width: 130,
    ...
    },
    {
      ...
      render: (_v, record) => {
        ...
        if (onEdit && isBundleExplosionDraft(record?.status)) {
          items.push({ key: "edit", icon: <EditOutlined />, label: t("actionEdit"), onClick: () => onEdit(record) });
        }
        if (onDelete && isBundleExplosionDraft(record?.status)) {
          items.push({
            key: "delete",
            icon: <DeleteOutlined />,
            danger: true,

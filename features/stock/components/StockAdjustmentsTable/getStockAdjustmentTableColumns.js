...
export function getStockAdjustmentTableColumns(t, actions = {}) {
  ...
  return [
    {
      title: t("colAdjNumber"),
      dataIndex: "adj_number",
      key: "adj_number",
      width: 140,
      render: (value) => value || "—",
    },
    ...
    {
      title: t("colAdjReason"),
      key: "reason",
      width: 180,
      ellipsis: true,
      render: (_v, record) => record?.reason?.name ?? "—",
    },
    {
      title: t("colStatus"),
      dataIndex: "status",
    ...
    },
    {
      ...
      render: (_v, record) => {
        ...
        if (onEdit && isStockAdjustmentDraft(record?.status)) {
          items.push({ key: "edit", icon: <EditOutlined />, label: t("actionEdit"), onClick: () => onEdit(record) });
        }
        if (onDelete && isStockAdjustmentDraft(record?.status)) {
          items.push({
            key: "delete",
            icon: <DeleteOutlined />,
            danger: true,

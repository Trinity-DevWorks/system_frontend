...
import { STOCK_COUNT_STATUS_VALUES, getStockCountStatusLabel } from "../utils/stockCountStatuses";
import {
...
import StockCountDrawer from "../components/StockCountDrawer/StockCountDrawer";
import { getStockCountTableColumns } from "../components/StockCountsTable/getStockCountTableColumns";
import { useStockCountsTableQuery } from "../queries/useStockCountsTableQuery";
...
function StockCountsTable() {
  ...
  const statusFilterOptions = useMemo(
    () => [
      ...
      ...STOCK_COUNT_STATUS_VALUES.map((value) => ({
        value,
        label: getStockCountStatusLabel(t, value),
      })),
    ],
    [t],
  );
  ...
  const tableData = useMemo(
    () =>
      rawTableData.map((row) => ({
        ...row,
        status_label: getStockCountStatusLabel(t, row?.status),
      })),
    [rawTableData, t],
  );
  ...
  const deleteMutation = useMutation({
    mutationFn: (/** @type {string} */ id) => deleteStockCount(id),
    onSuccess: () => {
  ...
  const handleDelete = useCallback(
    (record) => {
      ...
      if (id == null) return;
      const name = typeof record.cnt_number === "string" ? record.cnt_number : id;
      modal.confirm({
        title: t("cntDeleteConfirmTitle"),
        content: t("cntDeleteConfirmContent", { name }),

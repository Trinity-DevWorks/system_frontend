...
import { deleteBundleExplosion } from "../api/bundleExplosions.api";
...
import { BUNDLE_EXPLOSION_STATUS_VALUES, getBundleExplosionStatusLabel } from "../utils/bundleExplosionStatuses";
import {
...
function BundleExplosionsTable() {
  ...
  const statusFilterOptions = useMemo(
    () => [
      ...
      ...BUNDLE_EXPLOSION_STATUS_VALUES.map((value) => ({
        value,
        label: getBundleExplosionStatusLabel(t, value),
      })),
    ],
    [t],
  );
  ...
  const tableData = useMemo(
    () =>
      rawTableData.map((row) => ({
        ...row,
        status_label: getBundleExplosionStatusLabel(t, row?.status),
      })),
    [rawTableData, t],
  );
  ...
  const handleDelete = useCallback(
    (record) => {
      ...
      if (id == null) return;
      const name = typeof record.bex_number === "string" ? record.bex_number : id;
      modal.confirm({
        title: t("bexDeleteConfirmTitle"),
        content: t("bexDeleteConfirmContent", { name }),
  ...
  const columns = useMemo(
    () =>
      getBundleExplosionTableColumns(t, {
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col">
      <AppDataTable
        ...
        onRetry={() => refetch()}
        emptyText={t("bundleExplosionsEmpty")}
        toolbar={{
          showSearch: true,
...
export default function BundleExplosionsPage() {
  return (

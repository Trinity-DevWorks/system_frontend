...
import { deleteOpeningStock } from "../api/openingStocks.api";
...
import { OPENING_STOCK_STATUS_VALUES, getOpeningStockStatusLabel } from "../utils/openingStockStatuses";
import {
...
function OpeningStocksTable() {
  ...
  const statusFilterOptions = useMemo(
    () => [
      ...
      ...OPENING_STOCK_STATUS_VALUES.map((value) => ({
        value,
        label: getOpeningStockStatusLabel(t, value),
      })),
    ],
    [t],
  );
  ...
  const tableData = useMemo(
    () =>
      rawTableData.map((row) => ({
        ...row,
        status_label: getOpeningStockStatusLabel(t, row?.status),
      })),
    [rawTableData, t],
  );
  ...
  const handleDelete = useCallback(
    (record) => {
      ...
      if (id == null) return;
      const name = typeof record.os_number === "string" ? record.os_number : id;
      modal.confirm({
        title: t("osDeleteConfirmTitle"),
        content: t("osDeleteConfirmContent", { name }),
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col">
      <AppDataTable
        ...
        onRetry={() => refetch()}
        emptyText={t("openingStocksEmpty")}
        toolbar={{
          showSearch: true,

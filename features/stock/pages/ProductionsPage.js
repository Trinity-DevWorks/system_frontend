...
import { PRODUCTIONS_QUERY_KEY } from "../queries/stockQueryKeys";
import { getLocalizedApiErrorMessage } from "@/lib/api-error-notify";
import { useResourceDrawerUrl } from "@/lib/drawer/useResourceDrawerUrl";
import { normalizeEntityId } from "@/lib/entityId";
import { useResourceAccess } from "@/lib/permissions";
import { dayjsDatePattern } from "@/lib/tenant-format";
import { deleteProduction } from "../api/productions.api";
...
import { PRODUCTION_STATUS_VALUES, getProductionStatusLabel } from "../utils/productionStatuses";
import {
...
import ProductionDrawer from "../components/ProductionDrawer/ProductionDrawer";
import { getProductionTableColumns } from "../components/ProductionsTable/getProductionTableColumns";
import { useProductionsTableQuery } from "../queries/useProductionsTableQuery";
...
function ProductionsTable() {
  ...
  const statusFilterOptions = useMemo(
    () => [
      ...
      ...PRODUCTION_STATUS_VALUES.map((value) => ({
        value,
        label: getProductionStatusLabel(t, value),
      })),
    ],
    [t],
  );
  ...
  const tableData = useMemo(
    () =>
      rawTableData.map((row) => ({
        ...row,
        status_label: getProductionStatusLabel(t, row?.status),
      })),
    [rawTableData, t],
  );
  ...
  const deleteMutation = useMutation({
    mutationFn: (/** @type {string} */ id) => deleteProduction(id),
    onSuccess: () => {
  ...
  const handleDelete = useCallback(
    (record) => {
      ...
      if (id == null) return;
      const name = typeof record.prd_number === "string" ? record.prd_number : id;
      modal.confirm({
        title: t("prdDeleteConfirmTitle"),
        content: t("prdDeleteConfirmContent", { name }),

/**
 * Lookup queries for the stock count drawer.
 */
 ...
 */
export function useStockCountDrawerData({ open }) {
  ...
  const defaultWarehouseId = useMemo(() => {
    ...
    const preferred = active.find((w) => w?.is_default);
    ...
    () =>
      (itemsQuery.data ?? [])
        ...
        .map((item) => ({
          ...
          item_uom_label:
            typeof item.base_uom?.code === "string"
              ? item.base_uom.code
              : typeof item.base_uom?.name === "string"
                ? item.base_uom.name
                : "",
        })),
    [itemsQuery.data],

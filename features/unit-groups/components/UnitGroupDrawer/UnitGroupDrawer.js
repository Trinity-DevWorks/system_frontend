...
import { fetchUnitGroup } from "../../api/unitGroups.api";
import { App, Form } from "antd";
import { useTranslations } from "next-intl";
import { useCallback, useMemo } from "react";
import UnitGroupDrawerForm from "./UnitGroupDrawerForm";
import { useUnitGroupDrawerMutations } from "../../queries/useUnitGroupMutations";
import {
...
import { UNIT_GROUPS_LIST_QUERY_KEY } from "../../queries/unitGroupsQueryKeys";

const UNIT_GROUP_DETAIL_QUERY_PREFIX = /** @type {const} */ (UNIT_GROUPS_LIST_QUERY_KEY);
...
export default function UnitGroupDrawer({
  ...
  const lastCreateIntent = usePersistedSaveIntent(UNIT_GROUP_CREATE_SAVE_INTENT_KEY, UNIT_GROUP_CREATE_SAVE_INTENT_EVENT);
  ...
  const mapSeedToCacheRow = useCallback((seed) => toUnitGroupCacheRow(seed), []);
  ...
  const { detailEnabled, tableSeedMatches, fetchRemoteDetail, detailQuery } = useResourceDrawerDetailSync({
    open,
    mode,
    recordId: unitGroupId,
    tableSeedRecord,
    form,
    defaults,
    queryKeyPrefix: UNIT_GROUP_DETAIL_QUERY_PREFIX,
    fetchDetail: fetchUnitGroup,
    mapSeedToCacheRow,
  ...
  const dimensionTypeWatch = Form.useWatch("dimension_type", form);

  const canSubmitRequired = useMemo(() => {
    const code = typeof codeWatch === "string" ? codeWatch : "";
    const name = typeof nameWatch === "string" ? nameWatch : "";
    const dimensionType = typeof dimensionTypeWatch === "string" ? dimensionTypeWatch : "";
  ...
  const editBaselineForDirty = useMemo(() => {
    ...
    if (tableSeedMatches && tableSeedRecord) {
      return toUnitGroupCacheRow(/** @type {Record<string, unknown>} */ (tableSeedRecord));
    }
    return null;
  ...
  const handleEditSubmit = useCallback(() => {
    ...
    form
      ...
      .then((values) => {
        const payload = applyPayload(values);
        if (mode === "edit" && unitGroupId != null) {
          updateMutation.mutate({ id: unitGroupId, values: payload });
        }
      })
      .catch(() => {});
  }, [readOnly, form, applyPayload, mode, unitGroupId, updateMutation]);
  ...
  return (
    ...
    >
      <UnitGroupDrawerForm form={form} readOnly={readOnly} t={t} />
    </ResourceCrudDrawer>
  );
}

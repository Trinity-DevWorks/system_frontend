...
import { SUPPLIERS_LIST_QUERY_KEY, supplierDetailQueryKey } from "./suppliersQueryKeys";
...
export function useSupplierDrawerMutations({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data, variables, context) => {
      ...
      if (id != null) {
        queryClient.setQueryData(supplierDetailQueryKey(id), data);
      }
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (data, { id }) => {
      const listKey = SUPPLIERS_LIST_QUERY_KEY;
      const detailKey = supplierDetailQueryKey(id);
      patchTenantListCache(queryClient, listKey, (rows) => sortSuppliersByName(rows.map((row) => (row.id === id ? data : row))));

...
import { USERS_LIST_QUERY_KEY, userDetailQueryKey } from "../queries/tenantUsersQueryKeys";
...
export default function UserAvatarSection({
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: async (uploaded) => {
      ...
      if (userId && brief?.id) {
        queryClient.setQueryData(userDetailQueryKey(userId), (old) => {
          if (!old || typeof old !== "object") return old;
          return { ...old, avatar: brief };
        });
        patchTenantListCache(queryClient, USERS_LIST_QUERY_KEY, (rows) =>
          rows.map((row) =>

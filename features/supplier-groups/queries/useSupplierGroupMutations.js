...
} from "../utils/supplierGroupDrawerUtils";
import { SUPPLIER_GROUPS_LIST_QUERY_KEY, supplierGroupDetailQueryKey } from "./supplierGroupsQueryKeys";
 ...
 * }} args
 */
export function useSupplierGroupDrawerMutations({
  form,
  message,
  t,
  tApiErrors,
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data, variables, context) => {
      ...
      if (id != null) {
        queryClient.setQueryData(supplierGroupDetailQueryKey(id), data);
      }
  ...
  const updateMutation = useMutation({
    ...
    onError: (err, _variables, context) => {
      ...
      if (context?.previousDetail !== undefined) {
        queryClient.setQueryData(supplierGroupDetailQueryKey(context.id), context.previousDetail);
      }
      if (!applyApiFieldErrors(form, err)) {
        message.error(getLocalizedApiErrorMessage(tApiErrors, err));
      }
    ...
    onSuccess: (data, { id }) => {
      const listKey = SUPPLIER_GROUPS_LIST_QUERY_KEY;
      const detailKey = supplierGroupDetailQueryKey(id);
      patchTenantListCache(queryClient, listKey, (rows) => sortSupplierGroupsByName(rows.map((row) => (row.id === id ? data : row))));
